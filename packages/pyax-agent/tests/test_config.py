...
class TestConfigValidation:
    ...
    def test_invalid_permission_mode(self):
        config = AgentConfig(permission_mode="invalid")
        errors = config.validate()
        assert any("permission_mode" in e for e in errors)

    def test_multiple_errors(self):
        config = AgentConfig(max_turns=0, request_timeout=-1)
        errors = config.validate()
        assert len(errors) >= 2  # max_turns + request_timeout
